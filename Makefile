...
target = CUDA         # CUDA using NVSHMEM
...
## use_qxs_fp16_arch = {yes, no}
use_qxs_fp16 = no

## --- Accel branch ---
# use_alt_accel = {yes, no}
use_alt_accel = yes

# use_accel_impl = {openacc, cuda, nvshmem}
# device code implementation (currently only openacc is available).
use_accel_impl = cuda

# use_device_arch = {kepler, pascal, volta, ampere, hopper, rumors, auto}
# device architecture.
# auto: using nvidia-smi, which might not be available in some environments
#       (such as on systems at Univ. Tsukuba).
use_device_arch = ampere

# use_direct_comm = {yes , no}
# Set yes to use direct communication between GPU for supercomputer env.
# Yes is only for cuda, no will use pinned memory for both cuda and openacc
use_direct_comm = no
...
# simd_arch = {AVX1, AVX2, AVX512}
# only for use_alt_code = simd case
simd_arch = AVX512

## --- SIMD2 branch ---
# use_alt_simd2 = {yes, no}
use_alt_simd2 = no

# use_simd2_arch = {general, avx512}
use_simd2_arch = general
#use_simd2_arch = avx512

## --- Vector branch ---
# use_alt_vector = {yes, no}
use_alt_vector = no

# use_vector_impl = {necsx}
use_vector_impl = necsx
...
ifeq ($(use_alt_accel),yes)
  ifeq ($(use_accel_impl),cuda)
    EXTRA_LIBS += -lbridgecuda
  endif
  ifeq ($(use_accel_impl),nvshmem)
    EXTRA_LIBS += -lbridgecuda
  endif
endif
...
ifeq ($(use_qxs_fp16),yes)
  CPP_DEFS += -DUSE_FP16 -DUSE_QXS_FP16
endif
...
ifeq ($(use_alt_accel),yes)
 alt_inc_dirs += -Iinclude/bridge/lib_alt_Accel
 CPP_DEFS += -DUSE_ALT_ACCEL
ifeq ($(use_accel_impl),openacc)
 CPP_DEFS += -DUSE_ACCEL_OPENACC
endif
ifeq ($(use_accel_impl),cuda)
 CPP_DEFS += -DUSE_ACCEL_CUDA
 ifeq ($(use_direct_comm),yes)
 CPP_DEFS += -DUSE_DIRECT_COMM
endif
endif
ifeq ($(use_accel_impl),nvshmem)
 CPP_DEFS += -DUSE_ACCEL_NVSHMEM
endif
endif
ifeq ($(use_alt_simd),yes)
 alt_inc_dirs += -Iinclude/bridge/lib_alt_SIMD
 CPP_DEFS += -DUSE_ALT_SIMD -D$(simd_arch)
# CXXFLAGS += -fno-strict-aliasing
endif
ifeq ($(use_alt_simd2),yes)
 alt_inc_dirs += -Iinclude/bridge/lib_alt_SIMD2
 CPP_DEFS += -DUSE_ALT_SIMD2
endif
ifeq ($(use_alt_vector),yes)
 alt_inc_dirs += -Iinclude/bridge/lib_alt_Vector
 CPP_DEFS += -DUSE_ALT_VECTOR
ifeq ($(use_vector_impl),necsx)
 CPP_DEFS += -DUSE_VECTOR_NECSX
endif
endif
endif
...
all: msg lib program-with-lib build_bridgecuda all-done
...
ifeq ($(use_alt_accel),yes)
ifeq ($(use_accel_impl),openacc)
	cd ./src/lib_alt_Accel/; ln -s BridgeOpenACC BridgeACC ; cd ../../
endif
ifeq ($(use_accel_impl),cuda)
	cd ./src/lib_alt_Accel/; ln -s BridgeCUDA BridgeACC ; cd ../../
endif
ifeq ($(use_accel_impl),nvshmem)
	cd ./src/lib_alt_Accel/; ln -s BridgeNVSHMEM BridgeACC ; cd ../../
endif
endif
ifeq ($(use_alt_vector),yes)
ifeq ($(use_vector_impl),necsx)
	cd ./src/lib_alt_Vector/; ln -s BridgeSX BridgeVec ; cd ../../
endif
endif

ifeq ($(use_alt_simd2),yes)
ifeq ($(use_simd2_arch),avx512)
	cd ./src/lib_alt_SIMD2/; ln -s inline_AVX512 inline ; cd ../../
endif
ifeq ($(use_simd2_arch),general)
	cd ./src/lib_alt_SIMD2/; ln -s inline_General inline ; cd ../../
endif
endif
...
clean:
	...
	$(MAKE) delete_links
ifeq ($(use_alt_accel),yes)
	rm -f src/lib_alt_Accel/BridgeACC
  ifeq ($(use_accel_impl),cuda)
	$(MAKE) -C src/lib_alt_Accel/BridgeCUDA clean
	rm -f $(build_dir)/libbridgecuda.a
  endif
  ifeq ($(use_accel_impl),nvshmem)
	$(MAKE) -C src/lib_alt_Accel/BridgeNVSHMEM clean
	rm -f $(build_dir)/libbridgecuda.a
  endif
endif

#	
# for CUDA compilation (W-L.Chen added)
#.PHONY: build_bridgecuda

build_bridgecuda:
ifeq ($(use_alt_accel),yes)
ifeq ($(use_accel_impl),cuda)
	@echo "Starting build_bridgecuda..."
#	$(MAKE) -C src/lib_alt_Accel/BridgeCUDA clean
	$(MAKE) -C src/lib_alt_Accel/BridgeCUDA all
	@echo "Creating symbolic link for libbridgecuda.a..."
	rm -f $(build_dir)/libbridgecuda.a
	ln -s ../src/lib_alt_Accel/BridgeCUDA/libbridgecuda.a $(build_dir)/libbridgecuda.a
	@echo "Finished building BridgeCUDA"
endif
ifeq ($(use_accel_impl),nvshmem)
	@echo "Starting build_bridgecuda..."
#	$(MAKE) -C src/lib_alt_Accel/BridgeNVSHMEM clean
	$(MAKE) -C src/lib_alt_Accel/BridgeNVSHMEM all
	@echo "Creating symbolic link for libbridgecuda.a..."
	rm -f $(build_dir)/libbridgecuda.a
	ln -s ../src/lib_alt_Accel/BridgeNVSHMEM/libbridgecuda.a $(build_dir)/libbridgecuda.a
	@echo "Finished building BridgeNVSHMEM"
endif
endif
...
#        @date    $LastChangedDate:: 2024-11-05 17:41:16 #$
#        @version $LastChangedRevision: 2619 $
################################################################################
