...
template<typename AFIELD> class BlockFGMRES_dw;   // dev8 block-GMRES (fwd decl)
...
class ASolver_MG_dw : public ASolver<AFIELD>
{
 ...
 public:
  ...
  //! verify the MRHS / tensor-core transfer kernels against the production
  //! single-RHS make_coarse_vector / make_fine_vector on the LIVE testvectors.
  //! Call after run_setup().  AMG-local diagnostic (dev8 only).
  void verify_mrhs_transfer(int nrhs);

  //! #15: verify the batched MRHS coarse Dirac (coarse_mrhs) reproduces the
  //! production per-column coarse D (m_afopr_coarse->mult, mode "D") on nrhs
  //! random coarse vectors.  Foundation for batching the V-cycle coarse solve.
  //! AMG-local diagnostic (dev8 only); call after run_setup().
  void verify_coarse_mrhs(int nrhs);

  //! #15: batched coarse SOLVE -- coarse_prec_mrhs (Clov^-1 on all s) + a block
  //! coarse GMRES (BlockFGMRES_dw, block_A = coarse_mrhs = D) solving D X = b for
  //! s columns at once; verify vs the per-column production coarse solve.
  //! AMG-local diagnostic (dev8 only); call after run_setup().
  void verify_coarse_solve_block(int s);

  //! drive the dev8 flexible block-GMRES (s RHS at once) on the float A operator,
  //! preconditioned by the float V-cycle, and report per-column true residuals.
  //! AMG-local diagnostic (dev8 only); call after run_setup().
  //! do_ab=true additionally re-runs the WHOLE solve on the FP32 CUDA-core path
  //! and prints RESULT_TC_AB (TF32-vs-FP32 precision A/B) -- that doubles the
  //! runtime, so it defaults OFF (single full TF32 solve = the capstone).
  void verify_block_fgmres(int s, bool do_ab = false);

  //! batched (MRHS) two-grid V-cycle: applies the SAME V-cycle as the precond's
  //! mult_single to s columns at once, with restriction/prolongation done as the
  //! verified MRHS GEMMs (restrict_mrhs/prolong_mrhs); smoother + coarse solve
  //! stay per column (production solvers, via a coarse-layout repack).  dev8-local.
  void apply_vcycle_block(std::vector<AFIELD_f>& v,
                          const std::vector<AFIELD_f>& w);

  //! verify apply_vcycle_block reproduces per-column mult_single on s columns.
  void verify_vcycle_block(int s);

  //! batched (MRHS) fine A operator applied to all s columns at once:
  //! out_c = A in_c = (D_PV C_PV^-1)^dag C^-1 D in_c, via the on-device MRHS
  //! chain (fineD/finePrec/fineDdag/finePrecdag).  Reaches the two inner DW
  //! operators through the PVprec A (m_afopr_A_F).  dev8-local.
  void apply_A_block(std::vector<AFIELD_f>& out,
                     const std::vector<AFIELD_f>& in);

  //! batched fine smoother: m_smoother_niter steps of block-GMRES-on-A (the
  //! batched MRHS A, no preconditioner) on all s columns at once, zero start.
  ...
  //! propagator and report the physical residual.  dev8-local diagnostic.
  void verify_block_propagator(int s);

  //! production-grade batched propagator solve: psi[c] = D^{-1} eta[c] for ALL
  //! columns at once through the FULLY batched AMG (block-FGMRES with batched A +
  //! batched on-device V-cycle), wrapped in the same double physical-residual
  //! iterative refinement as the production 2pt.  This is the path that feeds the
  //! batched Hadron 2pt.  Returns the worst per-column physical residual
  //! sqrt(||D psi - eta||^2/||eta||^2) and the per-column refinement counts.
  void solve_block_propagator(std::vector<AFIELD>& psi,
 ...
 protected:
  ...
  //! cached pre-projected basis B' (2*nvec fields: P-/P+ v_i) for the MRHS
  //! V-cycle restriction/prolongation; built lazily on first apply_vcycle_block.
  ...
  //! cached coarse_dev buffer for apply_vcycle_block (alloc once, reuse).
  float *m_vc_coarse_dev = nullptr;
  long   m_vc_coarse_cap = 0;
  ...
  //! m_block_smoother.  Replaces the per-column m_asolver_coarse loop.
  std::unique_ptr<BlockFGMRES_dw<AFIELD_f> > m_block_coarse;
  std::vector<AFIELD_f> m_vc_cw, m_vc_cb, m_vc_cX;  //!< restricted res / prec'd / solution
  int m_vc_coarse_s = -1;                           //!< cached column count
  //! persistent fine-grid V-cycle scratch (residual r, prolong/post-smooth t),
  //! built once per s so apply_vcycle_block allocates NO fields per V-cycle.
  std::vector<AFIELD_f> m_vc_r, m_vc_t;
  int m_vc_rt_s = -1;
