...
{
  vout.paranoiac("%s: make_fine_vector is called\n", class_name.c_str());
...
#pragma omp barrier
  ...
  for (int ivec = 0; ivec < num_vectors; ++ivec) {
    // imple-2:  m_tmp1 = gm5 v[i]  (cached, no per-call gm5 apply)
...
{
  vout.paranoiac("%s: make_coarse_vector is called\n", class_name.c_str());
  ...
  for (int ivec = 0; ivec < num_vectors; ++ivec) {
    ...
    copy(m_tmp2, m_gm5_testvec[ivec]);          // gm5 v[i] (cached, no gm5 apply)
#pragma omp barrier
