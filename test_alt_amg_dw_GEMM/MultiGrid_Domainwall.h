/*!
        ...
        @file    MultiGrid_Domainwall.h

        @brief   MultiGrid operation for Domainwall fermion (template version)
...
//====================================================================
#ifndef MULTIGRID_DOMAINWALL_H_INCLUDED
#define MULTIGRID_DOMAINWALL_H_INCLUDED
...
class MultiGrid_Domainwall : public MultiGrid<AFIELD1, AFIELD2>
{
 public:
  ...
  typedef AFopr_Domainwall_5din_dd<AFIELD2> Afopr_fine_dd_t;
 ...
 protected:
  ...
  // not re-apply the fine domain-wall gm5 operator on every single call.
 ...
 public:
  MultiGrid_Domainwall() {}
  MultiGrid_Domainwall(const std::vector<int>& coarse_lattice,
                   const std::vector<int>& fine_lattice,
                   const int nin, const int nvec)
  { init(coarse_lattice, fine_lattice, nin, nvec); }

  ~MultiGrid_Domainwall() { tidyup(); }
  ...
  void set_afopr_fine(AFopr_dd<AFIELD2> *afopr)
  {
    ...
    vout.crucial("MultiGrid_Domainwall: bad afopr: only AFopr_Domainwall_5din_dd is vaild\n");
    exit(EXIT_FAILURE);
  }
