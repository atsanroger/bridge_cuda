                                            ...
                                            std::string mode)
{
  ...
  AStaple_lex<AField<double,IMPL> > staple_alt;
  double plaq;
  staple_alt.plaquette(plaq, *U.get());
  vout.general(m_vl, "plaq(alt) = %f\n", plaq);
  vout.general(m_vl, "\n");

  // smearing (old fashioned)
  /*
  Parameters  proj_params = params_all.lookup("Projection");
  std::string proj_type   = proj_params.get_string("projection_type");

  Parameters  smear_params = params_all.lookup("Smear");
  std::string smear_type   = smear_params.get_string("smear_type");

  Parameters  director_params = params_all.lookup("Director_Smear");
  int Nsmear = director_params.get_int("number_of_smearing");

  if (Nsmear > 0) {
    unique_ptr<Projection> proj(Projection::New(proj_type, proj_params));
    unique_ptr<Smear> smear(Smear::New(smear_type, proj.get(), smear_params));
    unique_ptr<Director_Smear> dr_smear(new Director_Smear(smear.get(), director_params));
    dr_smear->set_config(U.get());
  ...
  */
  ...
  Parameters  smear_params = params_all.lookup("LinkSmearing");

  unique_ptr<Director_alt_Smear<AField<double,IMPL> > > dr_smear(
             new Director_alt_Smear<AField<double,IMPL> >(smear_params));
  /*
  unique_ptr<Director_alt_Smear<Field> > dr_smear(
             new Director_alt_Smear<Field>(smear_params));
  */

  // Field_G Usmear(Nvol, Ndim);
  // dr_smear->get_config(Usmear);
  // copy(*U.get(), Usmear);

  //dr_smear->set_config(U.get());
  //copy(*U.get(), *(dr_smear->get_config()));
  ...
  if(mode == "double"){
    ...
    fprop.reset(new Fprop_alt_Standard_lex<AField<double,IMPL> >(
                         params_fopr, params_solver, dr_smear.get()) );
