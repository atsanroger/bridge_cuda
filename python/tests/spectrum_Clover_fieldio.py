...
fio=FieldIO("Binary", IO_Format_Trivial)

#--- Propagator calculator
#
def calc_prop(fopr, source, filename):
    ...
    for ispin in range(nd):
        for icolor in range(nc):
            ...
            fname="{}_c{:d}_s{:d}.dat".format(filename, (icolor+1), (ispin+1))
            fio.write_file(x, fname)

    # print(pp)
    # return pp


def load_propagator(filename):
    nc=Common.Nc
    nd=Common.Nd
...
#--- calculate propagators and store to files

print("=== START PROPAGATOR CALCULATION ===")

calc_prop(Fopr("Clover",
...
print("=== START CORRELATOR CALCULATION ===")

pp1 = load_propagator("prop_1")
pp2 = load_propagator("prop_2")
