...
#ifdef USE_FP16
inline hcomplex cmplx(const _Float16 r, const _Float16 i) { return hcomplex(r, i); }
inline _Float16 abs(hcomplex v)
{ double vr = (double)real(v);
  double vi = (double)imag(v);
  double vsq = sqrt(vr * vr + vi * vi);
  return (_Float16)vsq;
}
#endif
