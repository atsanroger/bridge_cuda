/*!
        @file    solver_BiCGStab_Cmplx_fp16-tmpl.h
        @brief
...
void ASolver_BiCGStab_Cmplx<AFIELD>::init(void)
{
  ...
  //m_ecrit           = 1.e-32;
  m_ecrit           = 5.96e-8;  // limitation of FP16
  m_nconv           = -1;
...
void ASolver_BiCGStab_Cmplx<AFIELD>::set_parameters(const Parameters& params)
{
  ...
  if(Stop_cond < m_ecrit){  // limitation of FP16
...
#pragma omp master
  ...
  copy(m_t, b);
  double snorm2;
  b.norm2_double(snorm2);
  double snorm     = sqrt(snorm2);
  double snorm_inv = 1.0 / snorm;
  vout.detailed(m_vl, "  |b| = snorm = %22.15e\n", snorm);
  scal(m_t, real_t(snorm_inv));
  ...
  int    nconv = -1;
  double rr;
  solve_init_double(m_t, rr, coeff, snorm_inv, m_initial_mode);
  vout.detailed(m_vl, "  init: %22.15e\n", rr);

  for (int iter = 0; iter < m_Niter; ++iter) {
    int iflg = 0;
    solve_step_double(rr, iflg, coeff);
    vout.detailed(m_vl, "%6d  %22.15e\n", iter, rr);
    if (iflg != 0) { // restart
      copy(m_t, b);
      scal(m_t, real_t(snorm_inv));
      solve_init_double_GIVEN(m_t, rr, coeff, double(1.0));
  ...
  scal(xq, real_t(snorm));

  m_fopr->mult(m_t, xq);
  sub(m_t, b);
  //double diff2 = norm2(m_t);
  double diff2;
  m_t.norm2_double(diff2);
...
//====================================================================
template<typename AFIELD>
inline void ASolver_BiCGStab_Cmplx<AFIELD>::solve_init_double(const AFIELD& b,
                                                       double& rr,
                                                       dcoeff_t& prev,
                                                       const double scale,
                                                       const InitialGuess init_mode)
{
  if (init_mode == InitialGuess::RHS) {
    solve_init_double_RHS(b, rr, prev);
  } else if (init_mode == InitialGuess::GIVEN) {
    solve_init_double_GIVEN(b, rr, prev, scale);
  } else if (init_mode == InitialGuess::ZERO) {
    solve_init_double_ZERO(b, rr, prev);
  } else {
...
//====================================================================
template<typename AFIELD>
void ASolver_BiCGStab_Cmplx<AFIELD>::solve_init_double_RHS(const AFIELD& b,
                                                    double& rr,
                                                    dcoeff_t& prev)
{
  AFIELD& m_x = *m_px;
  ...
  {
    //double temp = norm2(m_x);
    double temp;
    m_x.norm2_double(temp);
  ...
  {
    //double temp = norm2(m_v);
    double temp;
    m_v.norm2_double(temp);
...
//====================================================================
template<typename AFIELD>
void ASolver_BiCGStab_Cmplx<AFIELD>::solve_init_double_GIVEN(const AFIELD& b,
                                                      double& rr,
                                                      dcoeff_t& prev,
                                                      const double scale)
{
...
//====================================================================
template<typename AFIELD>
void ASolver_BiCGStab_Cmplx<AFIELD>::solve_init_double_ZERO(const AFIELD& b,
                                                     double& rr,
                                                     dcoeff_t& prev)
{
  // x0 = 0
...
//====================================================================
template<typename AFIELD>
void ASolver_BiCGStab_Cmplx<AFIELD>::solve_step_double(double& rr, int& iflg,
                                                dcoeff_t& prev)
{
  using dcomplex_t = typename std::complex<double>;
  using complex_t = typename AFIELD::complex_t;
  const double Omega_tolerance = m_Omega_tolerance;
  AFIELD&      m_x             = *m_px;

  //complex_t rho = dotc(m_rh, m_r);
  dcomplex_t rho;
  m_rh.dotc_double(rho, m_r);
  ...
  if (fabs(rho) < m_ecrit) {
    ...
    double r2, rh2;
    m_r.norm2_double(r2);
    m_rh.norm2_double(rh2);
    vout.detailed(m_vl, "   |m_r|^2  = %23.16e\n", r2);
    vout.detailed(m_vl, "   |m_rh|^2 = %23.16e\n", rh2);
#endif
    return;
  }
  dcomplex_t bet = rho * prev.alpha / (prev.rho * prev.omega);

  // p = r + bet * (p - omega_prev * v);
  complex_t prev_omega_h = cmplx(real_t(real(prev.omega)),
                                 real_t(imag(prev.omega)) );
  //axpy(m_p, real_t(-prev.omega), m_v);
  axpy(m_p, -prev_omega_h, m_v);
  aypx(bet, m_p, m_r);
  ...
  //axpy(m_r, -alpha, m_v);
  {
   complex_t alpha_h = cmplx(real_t(real(alpha)), real_t(imag(alpha)) );
   axpy(m_r, -alpha_h, m_v);
  }
  m_fopr->mult(m_t, m_r);
  ...
  dcomplex_t omega_n;
  double    omega_d, s_norm2;
  //dotc_and_norm2(omega_n, omega_d, s_norm2, m_t, m_r);
  m_t.dotc_double(omega_n, m_r);
  m_t.norm2_double(omega_d);
  m_r.norm2_double(s_norm2);

  dcomplex_t omega = omega_n / omega_d;
  ...
  {
   complex_t omega_h = cmplx(real_t(real(omega)), real_t(imag(omega)) );
   complex_t alpha_h = cmplx(real_t(real(alpha)), real_t(imag(alpha)) );
   axpy(m_x, omega_h, m_r);
   axpy(m_x, alpha_h, m_p);

   // "r" below is the residual vector
   axpy(m_r, -omega_h, m_t);
  }
