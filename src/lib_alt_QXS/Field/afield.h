...
class AField<REALTYPE, QXS>
{
  ...
  */
  ...
  //! for FP16 solver implementation
  void norm2_double(double& rr) const;

  //! for FP16 solver implementation
  void dot_double(double& prod, const AField<real_t, QXS>&) const;

  //! for FP16 solver implementation
  void dotc_double(std::complex<double>&, const AField<real_t, QXS>&) const;
};

// copy between fields with different precisions.
void copy(AField<double, QXS>&, const AField<float, QXS>&);
void copy(AField<float, QXS>&, const AField<double, QXS>&);
