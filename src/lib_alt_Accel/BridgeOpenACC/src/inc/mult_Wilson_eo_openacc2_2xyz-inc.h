/*!
      @file    mult_Wilson_eo_openacc2_2xyz-inc.h
      @brief
    ...
    if(do_comm[0] > 0){
      ...
      if(ix == Nx-1 && keo == 1){
        ...
        for(int ivc = 0; ivc < NVC; ++ivc){
          vt1[ivc] = buf_xp[IDXBF(ivc, 0, iyzt2)];
          vt2[ivc] = buf_xp[IDXBF(ivc, 1, iyzt2)];
        }
      ...
      if(ix == 0 && keo == 0){
        ...
        for(int ic = 0; ic < NC; ++ic){
          ...
          wt1[0] = bc2 * buf_xm[IDXBF_R(ic, 0, iyzt2)];
          wt1[1] = bc2 * buf_xm[IDXBF_I(ic, 0, iyzt2)];
          wt2[0] = bc2 * buf_xm[IDXBF_R(ic, 1, iyzt2)];
          wt2[1] = bc2 * buf_xm[IDXBF_I(ic, 1, iyzt2)];
