...
const std::string AFopr_Clover_dd<AFIELD>::class_name
...
#pragma omp barrier
  ...
  int ith = ThreadManager::get_thread_id();
  if (ith == 0){
    //BridgeACC::set_block_config(m_Ublock.ptr(0), m_Nsize, m_block_size);
...
//====================================================================
template<typename AFIELD>
void AFopr_Clover_dd<AFIELD>::set_config_saved(Field* u)
{
  ThreadManager::assert_single_thread(class_name);
...
#pragma omp barrier
  ...
  if(ith == 0){
    BridgeACC::mult_clover_dd_dirac(v2, up, ct, v1,
                                    m_CKs, m_bc2,
...
#pragma omp barrier
  ...
  if(ith == 0){
    BridgeACC::mult_clover_D_dirac(v2, up, ct, v1,
                                   m_Nsize, m_bc2, m_CKs);
  }
...
//====================================================================
template<typename AFIELD>
void AFopr_Clover_dd<AFIELD>::scal_local(real_t *v, real_t a)
{
#pragma omp master
  {
    int Nin = 2 * m_Nc * m_Nd;
...
//====================================================================
template<typename AFIELD>
void AFopr_Clover_dd<AFIELD>::D_alt(AFIELD &v, const AFIELD &w)
{
  if(m_memory_saved){

    vout.crucial("%s: D_alt is not available in memory saved mode\n",
                 class_name.c_str());
    exit(EXIT_FAILURE);
...
//====================================================================
template<typename AFIELD>
void AFopr_Clover_dd<AFIELD>::H_alt(AFIELD &v, const AFIELD &w)
{
  D_alt(m_v2, w);
  mult_gm5(v, m_v2);
}
...
double AFopr_Clover_dd<AFIELD>::flop_count(const std::string mode)
{
  ...
  // <-- this infomation seems incorrect, should be updated.
