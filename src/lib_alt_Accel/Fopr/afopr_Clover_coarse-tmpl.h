/*!
      ...
      @date    $LastChangedDate:: 2023-12-17 17:12:52 #$
      @version $LastChangedRevision: 2560 $
*/
...
#include "lib_alt_Accel/BridgeACC/bridgeACC_Clover_coarse.h"
			 ...
			 int Ndf, int Ncol, int Ncol2)
{
			     ...
			     int Ndf, int Ncol2, int NcolH)
 {
   for(int i = 0; i < NcolH; ++i){
     int i1 = 2 * i;     // chirality -
     int i2 = 2 * i + 1; // chirality +
     real_t vt1r = 0.0;
     real_t vt1i = 0.0;
     real_t vt2r = 0.0;
     real_t vt2i = 0.0;
     ...
     v2[IDX2(Ncol2, 2*i1,   site)] += vt1r;
     v2[IDX2(Ncol2, 2*i1+1, site)] += vt1i;
     v2[IDX2(Ncol2, 2*i2,   site)] += vt2r;
     v2[IDX2(Ncol2, 2*i2+1, site)] += vt2i;
   }
 }
                                ...
                                const std::vector<AFIELD>& atestvec)
{
  ...
  // fine_afopr must be AFopr_Clover_dd
  AFopr_Clover_dd<AFIELD> *afopr_fine =
               dynamic_cast<AFopr_Clover_dd<AFIELD>* >(fine_afopr);
  if(afopr_fine == nullptr){
    vout.crucial("%s: bad fine operator is given to generate_coarse_op.\n",
                 class_name.c_str());
    exit(EXIT_FAILURE);
  }
...
#pragma omp barrier
  ...
  for(int i1 = 0; i1 < num_vectors; ++i1){
    for(int ch1 = -1; ch1 < 2; ch1 += 2){  // ch = -1, +1
      ...
      vout.crucial("i1 = %d  ch1 = %d\n", i1, ch1);

      vout.crucial("  atestvec norm2 = %f\n", atestvec[i1].norm2());

      afopr_fine->mult_gm5(workvec1, atestvec[i1]);
      aypx(real_t(ch1), workvec1, atestvec[i1]);

      // diag block: "clover"
      afopr_fine->mult_dd(workvec2, workvec1);

      vout.crucial("  workvec1 norm2 = %f\n", workvec1.norm2());
      vout.crucial("  workvec2 norm2 = %f\n", workvec2.norm2());

      afopr_fine->mult_gm5(workvec3, workvec2);
      aypx(real_t(-1.0), workvec3, workvec2);

      afopr_fine->mult_gm5(workvec4, workvec2);
      aypx(real_t(1.0),  workvec4, workvec2);

      vout.crucial("  workvec3 norm2 = %f\n", workvec3.norm2());
      vout.crucial("  workvec4 norm2 = %f\n", workvec4.norm2());
...
#pragma omp barrier
      ...
      for(int i2 = 0; i2 < num_vectors; ++i2){
        ...
        block_dotc(&tmp_buffer2[0], atestvec[i2], workvec4,
                   index_block);
        ...
        for(int s = coarse_is; s < coarse_ns; ++s){
          ...
          out_csw[idx_r] += 0.25 * real(tmp_buffer1[s]);
          out_csw[idx_i] += 0.25 * imag(tmp_buffer1[s]);
        }
        ...
        for(int s = coarse_is; s < coarse_ns; ++s){
          int idx_r = index_coarse.idx_Gr(J, I, s, 0);
          int idx_i = index_coarse.idx_Gi(J, I, s, 0);
          out_csw[idx_r] += 0.25 * real(tmp_buffer2[s]);
          out_csw[idx_i] += 0.25 * imag(tmp_buffer2[s]);
        }
      } // i2
...
#pragma omp barrier
      ...
      for(int mu = 0; mu < 4; ++mu){
        ...
        workvec2.set(0.0);
        timer2.start();
        afopr_fine->mult_dup(workvec2, workvec1, mu);
        timer2.stop();
...
#pragma omp barrier
        ...
        for(int i2 = 0; i2 < num_vectors; ++i2){
          ...
          for(int s = coarse_is; s < coarse_ns; ++s){
            ...
            out_gauge[idx_r] += 0.25 * real(tmp_buffer1[s]);
            out_gauge[idx_i] += 0.25 * imag(tmp_buffer1[s]);
          }
          ...
          for(int s = coarse_is; s < coarse_ns; ++s){
            int idx_r = index_coarse.idx_Gr(J, I, s, mu);
            int idx_i = index_coarse.idx_Gi(J, I, s, mu);
            out_gauge[idx_r] += 0.25 * real(tmp_buffer2[s]);
            out_gauge[idx_i] += 0.25 * imag(tmp_buffer2[s]);
          }
        } // i2
...
#pragma omp barrier
 ...
 vout.general("mu = 0\n");
 {
  for(int site = 0; site < coarse_nvol; ++site){
    {
      int j = 1;
      int i = 2;
      int mu = 0;
      int idx_r = index_coarse.idx_Gr(j, i, site, mu);
      int idx_i = index_coarse.idx_Gi(j, i, site, mu);
      vout.general("site = %d %f  %f\n", site,
 ...
 vout.general("clover term\n");
 {
  for(int i = 0; i < 2*num_vectors; ++i){
    for(int j = 0; j < 2*num_vectors; ++j){
      int site = 101;
      int idx_r = index_coarse.idx_Gr(j, i, site, 0);
      int idx_i = index_coarse.idx_Gi(j, i, site, 0);
      vout.general("i = %d  j = %d  %f  %f\n", i, j,
 ...
 //#endif
  ...
  double elapsed_time2 = timer2.elapsed_sec();
  vout.general(m_vl, "  mult_dup: %e sec\n", elapsed_time2);
...
#pragma omp barrier
  ...
  if(ith == 0){
    BridgeACC::mult_clover_coarse_mult_gm5(vp, wp, m_ncol, m_Nsize);
  }
...
//====================================================================
template<typename AFIELD>
void AFopr_Clover_coarse<AFIELD>::mult_gm5_host(real_t *v, real_t *w)
...
void AFopr_Clover_coarse<AFIELD>::mult_D(AFIELD &v, const AFIELD &w)
{
  ...
  if(ith == 0){
    ...
    if(do_comm_any > 0){
      ...
      BridgeACC::mult_clover_coarse_1(
                              buf1xp, buf1xm, buf1yp, buf1ym,
                              buf1zp, buf1zm, buf1tp, buf1tm,
                              u, v1, m_ncol, m_Nsize, do_comm);
      chset_send.start();
      chset_recv.start();
    }

    BridgeACC::mult_clover_coarse_bulk(v2, u, ct, v1,
    ...
    if(do_comm_any > 0){
      ...
      BridgeACC::mult_clover_coarse_2(v2, u,
...
//====================================================================
template<typename AFIELD>
void AFopr_Clover_coarse<AFIELD>::mult_csw_host(real_t *v2, real_t *v1)
{
...
//====================================================================
template<typename AFIELD>
void AFopr_Clover_coarse<AFIELD>::clear_host(real_t *v)
{
  int ith, nth, is, ns;
  set_threadtask_afopr(ith, nth, is, ns, m_Nst);
  ...
  if(do_comm[0] > 0){
    ...
    for(int site = is; site < ns; ++site){
      ...
      if(ix == m_Nx-1){
        ...
        accum_mult_udag(buf1, u, v1, iyzt, site, m_Ndf, m_Nvc, NcolH);
      }
    }
  ...
  for(int site = is; site < ns; ++site){
    ...
    if(ix > 0 || do_comm[0] == 0){
      ...
      if(ix == 0) nei = m_Nx-1 + m_Nx * iyzt;
        accum_mult_udag(v2, u, v1, site, nei, m_Ndf, m_Nvc, NcolH);
    }else{
      for(int ivc = 0; ivc < m_Nvc; ++ivc){
  ...
  if(do_comm[1] > 0){
    ...
    for(int site = is; site < ns; ++site){
      ...
      if(iy == m_Ny-1){
        ...
        accum_mult_udag(buf1, u, v1, ixzt, site, m_Ndf, m_Nvc, NcolH);
      }
    }
  ...
  if(do_comm[2] > 0){
    ...
    for(int site = is; site < ns; ++site){
      ...
      if(iz == m_Nz-1){
        ...
        accum_mult_udag(buf1, u, v1, ixyt, site, m_Ndf, m_Nvc, NcolH);
      }
    }
  ...
  if(do_comm[3] > 0){
    ...
    for(int site = is; site < ns; ++site){
      ...
      if(it == m_Nt-1){
        ...
        accum_mult_udag(buf1, u, v1, ixyz, site, m_Ndf, m_Nvc, NcolH);
      }
    }
