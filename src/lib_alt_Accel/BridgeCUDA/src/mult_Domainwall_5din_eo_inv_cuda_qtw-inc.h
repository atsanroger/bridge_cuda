    ...
    real_t* vp, real_t* wp, int Ns, int Nst, int Nst_pad, real_t alpha)
{
    ...
    const real_t* a_mid_con     = ConstantMemoryTraits<real_t>::a_mid();
    const real_t* a_lo_con      = ConstantMemoryTraits<real_t>::a_lo_tw();
...
// a[j] = 0.5*dm[j+1]*dpinv[j], precomputed (triple-word) in constant memory.
// Replaces the per-thread QTW_HALF_DMDP multiword multiply; needs a_con/a_mid_con/
// a_lo_con declared in the kernel. QTW_HALF_DMDP(name,is,is-1) -> QTW_LOAD_A(name,is-1);
// QTW_HALF_DMDP(name,is+1,is) -> QTW_LOAD_A(name,is).
#define QTW_LOAD_A(name, j) \
    const real_t name##_h = a_con[j]; \
    const real_t name##_m = a_mid_con[j]; \
    const real_t name##_l = a_lo_con[j]
...
// =================================================================
// v2 LU(dag)inv — forward-caching variants (templated on NS).
//   Cache the forward-sweep result in a thread-local vt_fwd[NS][ND][6]
//   triple-word array so the backward sweep does NOT round-trip the whole
//   5D ee-field through DRAM (forward STORE + backward LOAD of every site).
//   Renormalize3 is applied on cache-put so the cached value is exactly what
//   QTW_STORE6 would have written to vp -> bit-identical to the _fp kernel.
//   The is=NS-1 forward result is carried live in registers (not cached),
//   mirroring the FP v2 kernel.
//   Gated by -DUSE_LUINV_V2 in the .cu; launcher switch selects NS.  For QTW
//   the cache is NS*ND*6 reals (=192 at NS=8) and will spill to local memory,
//   but that per-thread-contiguous spill is far cheaper than the strided
//   global round-trip it removes.  This kernel is also a diagnostic: if it
//   speeds the LU up a lot the path is bandwidth-bound (round-trip dominated);
//   if it barely moves the path is compute-bound on the triple-word arithmetic.
// =================================================================

// Put renormalized named-scalar triple-word `prefix` into cache[is][id][0..5].
#define QTW_CACHE_PUT(cache, is, id, prefix) do { \
    real_t _cr_h = prefix##_rh, _cr_m = prefix##_rm, _cr_l = prefix##_rl; \
    real_t _ci_h = prefix##_ih, _ci_m = prefix##_im, _ci_l = prefix##_il; \
    Renormalize3(_cr_h, _cr_m, _cr_l); \
    Renormalize3(_ci_h, _ci_m, _ci_l); \
    (cache)[is][id][0] = _cr_h; (cache)[is][id][1] = _ci_h; \
    (cache)[is][id][2] = _cr_m; (cache)[is][id][3] = _ci_m; \
    (cache)[is][id][4] = _cr_l; (cache)[is][id][5] = _ci_l; \
} while (0)
// Declare named-scalar triple-word `prefix` from cache[is][id][0..5].
#define QTW_CACHE_GET(prefix, cache, is, id) \
    real_t prefix##_rh = (cache)[is][id][0]; \
    real_t prefix##_ih = (cache)[is][id][1]; \
    real_t prefix##_rm = (cache)[is][id][2]; \
    real_t prefix##_im = (cache)[is][id][3]; \
    real_t prefix##_rl = (cache)[is][id][4]; \
    real_t prefix##_il = (cache)[is][id][5]
// Renormalize a named-scalar triple-word in place.
#define QTW_RENORM(prefix) do { \
    Renormalize3(prefix##_rh, prefix##_rm, prefix##_rl); \
    Renormalize3(prefix##_ih, prefix##_im, prefix##_il); \
} while (0)

template <int NS>
__global__ void mult_domainwall_5din_ee_LUinv_dirac_qtw_kernel_v2(
    real_t* __restrict__ vp, real_t* __restrict__ wp, int Nst, int Nst_pad, real_t alpha)
{
    const int tid  = blockIdx.x * blockDim.x + threadIdx.x;
    const int site = tid % Nst_pad;
    const int ic   = tid / Nst_pad;
    ...
    // ===== Forward sweep: L^{-1} (cache results, no vp store) =====
    {
        QTW_LOAD6(_v0, wp, 6 * IDX_DWF_QTW(ic, 0, 0, NS, site));
        QTW_LOAD6(_v1, wp, 6 * IDX_DWF_QTW(ic, 1, 0, NS, site));
        QTW_LOAD6(_v2, wp, 6 * IDX_DWF_QTW(ic, 2, 0, NS, site));
    ...
    QTW_CACHE_PUT(vt_fwd, 0, 0, vt0);
    QTW_CACHE_PUT(vt_fwd, 0, 1, vt1);
    QTW_CACHE_PUT(vt_fwd, 0, 2, vt2);
    QTW_CACHE_PUT(vt_fwd, 0, 3, vt3);
    QTW_SCALC_TT(yt0, e_con[0], e_mid_con[0], e_lo_con[0], vt0);
    QTW_SCALC_TT(yt1, e_con[0], e_mid_con[0], e_lo_con[0], vt1);
    QTW_SCALC_TT(yt2, e_con[0], e_mid_con[0], e_lo_con[0], vt2);
    QTW_SCALC_TT(yt3, e_con[0], e_mid_con[0], e_lo_con[0], vt3);

    #pragma unroll 1   // rolled: QTW body is heavy; full unroll explodes ptxas time + register/local pressure
    for (int is = 1; is < NS-1; ++is) {
        QTW_LOAD_A(a, is-1);
        QTW_COPY(xt0, vt0); QTW_COPY(xt1, vt1); QTW_COPY(xt2, vt2); QTW_COPY(xt3, vt3);
        QTW_LOAD6(_v0, wp, 6 * IDX_DWF_QTW(ic, 0, is, NS, site));
        ...
        QTW_ADDC(sum, xt1, xt3); QTW_SCALC_TT(tmp, a_h, a_m, a_l, sum);
        QTW_ADDC(vt1, vt1, tmp); QTW_ADDC(vt3, vt3, tmp);
        QTW_CACHE_PUT(vt_fwd, is, 0, vt0);
        QTW_CACHE_PUT(vt_fwd, is, 1, vt1);
        QTW_CACHE_PUT(vt_fwd, is, 2, vt2);
        QTW_CACHE_PUT(vt_fwd, is, 3, vt3);
        QTW_SCALC_TT(tmp, e_con[is], e_mid_con[is], e_lo_con[is], vt0); QTW_ADDC(yt0, yt0, tmp);
        QTW_SCALC_TT(tmp, e_con[is], e_mid_con[is], e_lo_con[is], vt1); QTW_ADDC(yt1, yt1, tmp);
        QTW_SCALC_TT(tmp, e_con[is], e_mid_con[is], e_lo_con[is], vt2); QTW_ADDC(yt2, yt2, tmp);
        QTW_SCALC_TT(tmp, e_con[is], e_mid_con[is], e_lo_con[is], vt3); QTW_ADDC(yt3, yt3, tmp);
...

    {
        ...
        QTW_SUBC(diff, yt1, yt3); QTW_SCALC(tmp, real_t(0.5), diff);
        QTW_SUBC(vt1, vt1, tmp); QTW_ADDC(vt3, vt3, tmp);
        // is=NS-1 forward result carried live into the middle step (not cached).
    }

    // ===== Middle step: D^{-1} at is=NS-1 (uses live vt, no vp round-trip) =====
    {
        const int is = NS-1;
        const real_t aa_h = dpinv_con[NS-1], aa_m = dpinv_mid_con[NS-1], aa_l = dpinv_lo_con[NS-1];
        const real_t f1 = real_t(0.5) * (real_t(1.0) + alpha);
        const real_t f2 = real_t(0.5) * (real_t(-1.0) + alpha);
        QTW_RENORM(vt0); QTW_RENORM(vt1); QTW_RENORM(vt2); QTW_RENORM(vt3);
        QTW_DECL(_p0); QTW_DECL(_p1); QTW_DECL(_p2); QTW_DECL(_p3);
        QTW_COPY(_p0, vt0); QTW_COPY(_p1, vt1); QTW_COPY(_p2, vt2); QTW_COPY(_p3, vt3);
        QTW_DECL(t);
        QTW_SCALC(vt0, f1, _p0); QTW_SCALC(t, f2, _p2); QTW_ADDC(vt0, vt0, t); QTW_SCALC_TT(vt0, aa_h, aa_m, aa_l, vt0);
        QTW_SCALC(vt1, f1, _p1); QTW_SCALC(t, f2, _p3); QTW_ADDC(vt1, vt1, t); QTW_SCALC_TT(vt1, aa_h, aa_m, aa_l, vt1);
        QTW_SCALC(vt2, f1, _p2); QTW_SCALC(t, f2, _p0); QTW_ADDC(vt2, vt2, t); QTW_SCALC_TT(vt2, aa_h, aa_m, aa_l, vt2);
    ...
    // ===== Backward sweep: D^{-1} U^{-1} (read cache, no vp load) =====
    #pragma unroll 1   // rolled: QTW body is heavy; full unroll explodes ptxas time + register/local pressure
    for (int is = NS-2; is >= 0; --is) {
        QTW_HALF_DM(a, is);
        QTW_COPY(xt0, vt0); QTW_COPY(xt1, vt1); QTW_COPY(xt2, vt2); QTW_COPY(xt3, vt3);
        QTW_CACHE_GET(_v0, vt_fwd, is, 0);
        QTW_CACHE_GET(_v1, vt_fwd, is, 1);
        QTW_CACHE_GET(_v2, vt_fwd, is, 2);
        QTW_CACHE_GET(_v3, vt_fwd, is, 3);
        QTW_COPY(vt0, _v0); QTW_COPY(vt1, _v1); QTW_COPY(vt2, _v2); QTW_COPY(vt3, _v3);
        QTW_SUBC(diff, xt0, xt2); QTW_SCALC_TT(tmp, a_h, a_m, a_l, diff);
        QTW_ADDC(vt0, vt0, tmp);  QTW_SUBC(vt2, vt2, tmp);
        QTW_SUBC(diff, xt1, xt3); QTW_SCALC_TT(tmp, a_h, a_m, a_l, diff);
...
template <int NS>
__global__ void mult_domainwall_5din_ee_LUdaginv_dirac_qtw_kernel_v2(
    real_t* __restrict__ vp, real_t* __restrict__ wp, int Nst, int Nst_pad, real_t alpha)
{
    const int tid  = blockIdx.x * blockDim.x + threadIdx.x;
    const int site = tid % Nst_pad;
    ...
    const real_t* a_mid_con     = ConstantMemoryTraits<real_t>::a_mid();
    const real_t* a_lo_con      = ConstantMemoryTraits<real_t>::a_lo_tw();
    (void)e_mid_con; (void)e_lo_con;  // e unused in Udag^{-1} forward; kept for symmetry
    ...
    // ===== Forward sweep: Udag^{-1} * D^{-1} (cache results, no vp store) =====
    {
        const int is = 0;
        const real_t a0_h = dpinv_con[0], a0_m = dpinv_mid_con[0], a0_l = dpinv_lo_con[0];
        const real_t f1 = real_t(0.5) * (real_t(1.0) + alpha);
    ...
    // ===== Backward sweep: Ldag^{-1} (read cache, no vp load) =====
    #pragma unroll 1   // rolled: QTW body is heavy; full unroll explodes ptxas time + register/local pressure
    for (int is = NS-2; is >= 0; --is) {
        QTW_LOAD_A(a, is);
        QTW_COPY(xt0, vt0); QTW_COPY(xt1, vt1); QTW_COPY(xt2, vt2); QTW_COPY(xt3, vt3);
    ...
    real_t* vp, real_t* wp, int Ns, int* Nsize, double alpha, bool ext)
{
    ...
    QTW_PROF_BEGIN();
#ifdef USE_LUINV_V2
    #define LUINV_QTW_V2(NS) \
        mult_domainwall_5din_ee_LUinv_dirac_qtw_kernel_v2<NS><<<gridSize, blockSize>>>( \
            vp_dev, wp_dev, Nst, Nst_pad, (real_t)alpha)
    switch (Ns) {
        case  4: LUINV_QTW_V2( 4); break;
        case  6: LUINV_QTW_V2( 6); break;
        case  8: LUINV_QTW_V2( 8); break;
        case 10: LUINV_QTW_V2(10); break;
        case 12: LUINV_QTW_V2(12); break;
        case 14: LUINV_QTW_V2(14); break;
        case 16: LUINV_QTW_V2(16); break;
        case 18: LUINV_QTW_V2(18); break;
        case 20: LUINV_QTW_V2(20); break;
        case 22: LUINV_QTW_V2(22); break;
        case 24: LUINV_QTW_V2(24); break;
        case 32: LUINV_QTW_V2(32); break;
        default:
    ...
    real_t* vp, real_t* wp, int Ns, int* Nsize, double alpha, bool ext)
{
    ...
    QTW_PROF_BEGIN();
#ifdef USE_LUINV_V2
    #define LUDAGINV_QTW_V2(NS) \
        mult_domainwall_5din_ee_LUdaginv_dirac_qtw_kernel_v2<NS><<<gridSize, blockSize>>>( \
            vp_dev, wp_dev, Nst, Nst_pad, (real_t)alpha)
    switch (Ns) {
        case  4: LUDAGINV_QTW_V2( 4); break;
        case  6: LUDAGINV_QTW_V2( 6); break;
        case  8: LUDAGINV_QTW_V2( 8); break;
        case 10: LUDAGINV_QTW_V2(10); break;
        case 12: LUDAGINV_QTW_V2(12); break;
        case 14: LUDAGINV_QTW_V2(14); break;
        case 16: LUDAGINV_QTW_V2(16); break;
        case 18: LUDAGINV_QTW_V2(18); break;
        case 20: LUDAGINV_QTW_V2(20); break;
        case 22: LUDAGINV_QTW_V2(22); break;
        case 24: LUDAGINV_QTW_V2(24); break;
        case 32: LUDAGINV_QTW_V2(32); break;
        default:
            mult_domainwall_5din_ee_LUdaginv_dirac_qtw_kernel_fp<<<gridSize, blockSize>>>(
                vp_dev, wp_dev, Ns, Nst, Nst_pad, (real_t)alpha);
            break;
    }
    #undef LUDAGINV_QTW_V2
#else
    mult_domainwall_5din_ee_LUdaginv_dirac_qtw_kernel_fp<<<gridSize, blockSize>>>(
        vp_dev, wp_dev, Ns, Nst, Nst_pad, (real_t)alpha);
#endif
