...
void AFopr_Wilson<AFIELD>::init(const Parameters& params)
{
  ...
  // Nc check: temporary comment out.
  // check_Nc(m_Nc);
  // check_setup();

  int Nst = Nx * Ny * Nz * Nt;
  if(Nst % VLEN != 0){
    vout.crucial(m_vl, "%s: Nst (=%d) is not multiple of VLEN (= %d).\n",
                 class_name.c_str(), Nst, VLEN);
    exit(EXIT_FAILURE);
  }
...
void AFopr_Wilson<AFIELD>::mult_D(AFIELD &v, const AFIELD &w)
{
  ...
  if(do_comm_any > 0){
    ...
    if(m_repr == DIRAC){
      BridgeVec::mult_wilson_1_dirac(buf1xp, buf1xm, buf1yp, buf1ym, 
                          buf1zp, buf1zm, buf1tp, buf1tm,
                          u, v1, m_Nsize, m_bc, do_comm, NC);
    }else{
      BridgeVec::mult_wilson_1_chiral(buf1xp, buf1xm, buf1yp, buf1ym, 
                           buf1zp, buf1zm, buf1tp, buf1tm,
                           u, v1, m_Nsize, m_bc, do_comm, NC);
    }
  ...
  // bulk part
  if(m_repr == DIRAC){
    BridgeVec::mult_wilson_D_dirac(v2, u, v1, m_Nsize, m_bc2, m_CKs);
  }else{
    BridgeVec::mult_wilson_D_chiral(v2, u, v1, m_Nsize, m_bc2, m_CKs);
  }
...
//====================================================================
template<typename AFIELD>
void AFopr_Wilson<AFIELD>::mult_gm5(real_t *v, real_t *w)
{
  if(m_repr == DIRAC){
    BridgeVec::mult_wilson_gm5_dirac(v, w, m_Nsize, NC);
  }else{
    BridgeVec::mult_wilson_gm5_chiral(v, w, m_Nsize, NC);
  }
}

//====================================================================
template<typename AFIELD>
void AFopr_Wilson<AFIELD>::gm5_aypx(real_t a, real_t *v, real_t *w)
{
  if(m_repr == DIRAC){
    BridgeVec::mult_wilson_gm5_aypx_dirac(a, v, w, m_Nsize, NC);
  }else{
    BridgeVec::mult_wilson_gm5_aypx_chiral(a, v, w, m_Nsize, NC);
  }
}
...
{
  BridgeVec::mult_wilson_aypx(a, v, w, m_Nsize, NC);
}
...
{
  BridgeVec::mult_wilson_clear(v, m_Nsize, NC);
}
...
void AFopr_Wilson<AFIELD>::mult_xp(real_t *v2, real_t *v1)
{
  ...
  if(do_comm[idir] > 0){
    ...
    BridgeVec::mult_wilson_xp1(buf1, v1, m_Nsize, m_bc, NC);
    // copy_from_device(buf1, m_Nbdsize[idir]);
  ...
  BridgeVec::mult_wilson_xpb(v2, u, v1, m_Nsize, m_bc2, NC);

  if(do_comm[idir] > 0){
    // copy_to_device(buf2, m_Nbdsize[idir]);
    BridgeVec::mult_wilson_xp2(v2, u, buf2, m_Nsize, m_bc, NC);
  }
...
void AFopr_Wilson<AFIELD>::mult_xm(real_t *v2, real_t *v1)
{
  ...
  if(do_comm[idir] > 0){
    ...
    BridgeVec::mult_wilson_xm1(buf1, u, v1, m_Nsize, m_bc, NC);
    // copy_from_device(buf1, m_Nbdsize[idir]);
  ...
  BridgeVec::mult_wilson_xmb(v2, u, v1, m_Nsize, m_bc2, NC);

  if(do_comm[idir] > 0){
    // copy_to_device(buf2, m_Nbdsize[idir]);
    BridgeVec::mult_wilson_xm2(v2, buf2, m_Nsize, m_bc, NC);
  }
...
void AFopr_Wilson<AFIELD>::mult_yp(real_t *v2, real_t *v1)
{
  ...
  if(do_comm[idir] > 0){
    ...
    BridgeVec::mult_wilson_yp1(buf1, v1, m_Nsize, m_bc, NC);
    // copy_from_device(buf1, m_Nbdsize[idir]);
  ...
  BridgeVec::mult_wilson_ypb(v2, u, v1, m_Nsize, m_bc2, NC);

  if(do_comm[idir] > 0){
    // copy_to_device(buf2, m_Nbdsize[idir]);
    BridgeVec::mult_wilson_yp2(v2, u, buf2, m_Nsize, m_bc, NC);
  }
...
void AFopr_Wilson<AFIELD>::mult_ym(real_t *v2, real_t *v1)
{
  ...
  if(do_comm[idir] > 0){
    ...
    BridgeVec::mult_wilson_ym1(buf1, u, v1, m_Nsize, m_bc, NC);
    // copy_from_device(buf1, m_Nbdsize[idir]);
   ...
   BridgeVec::mult_wilson_ymb(v2, u, v1, m_Nsize, m_bc2, NC);

   if(do_comm[idir] > 0){
    // copy_to_device(buf2, m_Nbdsize[idir]);
    BridgeVec::mult_wilson_ym2(v2, buf2, m_Nsize, m_bc, NC);
   }
...
void AFopr_Wilson<AFIELD>::mult_zp(real_t *v2, real_t *v1)
{
  ...
  if(do_comm[idir] > 0){
    ...
    BridgeVec::mult_wilson_zp1(buf1, v1, m_Nsize, m_bc, NC);
    // copy_from_device(buf1, m_Nbdsize[idir]);
  ...
  BridgeVec::mult_wilson_zpb(v2, u, v1, m_Nsize, m_bc2, NC);

  if(do_comm[idir] > 0){
    // copy_to_device(buf2, m_Nbdsize[idir]);
    BridgeVec::mult_wilson_zp2(v2, u, buf2, m_Nsize, m_bc, NC);
  }
...
void AFopr_Wilson<AFIELD>::mult_zm(real_t *v2, real_t *v1)
{
  ...
  if(do_comm[idir] > 0){
    ...
    BridgeVec::mult_wilson_zm1(buf1, u, v1, m_Nsize, m_bc, NC);
    // copy_from_device(buf1, m_Nbdsize[idir]);
  ...
  BridgeVec::mult_wilson_zmb(v2, u, v1, m_Nsize, m_bc2, NC);

  if(do_comm[idir] > 0){
    // copy_to_device(buf2, m_Nbdsize[idir]);
    BridgeVec::mult_wilson_zm2(v2, buf2, m_Nsize, m_bc, NC);
  }
...
void AFopr_Wilson<AFIELD>::mult_tp(real_t *v2, real_t *v1)
{
  ...
  if(do_comm[idir] > 0){
    ...
    if(m_repr == DIRAC){
      BridgeVec::mult_wilson_tp1_dirac(buf1, v1, m_Nsize, m_bc, NC);
    }else{
      BridgeVec::mult_wilson_tp1_chiral(buf1, v1, m_Nsize, m_bc, NC);
    }
    // copy_from_device(buf1, m_Nbdsize[idir]);
  ...
  if(m_repr == DIRAC){
    BridgeVec::mult_wilson_tpb_dirac(v2, u, v1, m_Nsize, m_bc2, NC);
  }else{
    BridgeVec::mult_wilson_tpb_chiral(v2, u, v1, m_Nsize, m_bc2, NC);
  }

  if(do_comm[idir] > 0){
    // copy_to_device(buf2, m_Nbdsize[idir]);
    if(m_repr == DIRAC){
      BridgeVec::mult_wilson_tp2_dirac(v2, u, buf2, m_Nsize, m_bc, NC);
    }else{
      BridgeVec::mult_wilson_tp2_chiral(v2, u, buf2, m_Nsize, m_bc, NC);
    }
  }
...
void AFopr_Wilson<AFIELD>::mult_tm(real_t *v2, real_t *v1)
{
  ...
  if(do_comm[idir] > 0){
    ...
    if(m_repr == DIRAC){
      BridgeVec::mult_wilson_tm1_dirac(buf1, u, v1, m_Nsize, m_bc, NC);
    }else{
      BridgeVec::mult_wilson_tm1_chiral(buf1, u, v1, m_Nsize, m_bc, NC);
    }
    // copy_from_device(buf1, m_Nbdsize[idir]);
    ...
    if(m_repr == DIRAC){
      BridgeVec::mult_wilson_tmb_dirac(v2, u, v1, m_Nsize, m_bc2, NC);
    }else{
      BridgeVec::mult_wilson_tmb_chiral(v2, u, v1, m_Nsize, m_bc2, NC);
    }

  if(do_comm[idir] > 0){
    // copy_to_device(buf2, m_Nbdsize[idir]);
    if(m_repr == DIRAC){
      BridgeVec::mult_wilson_tm2_dirac(v2, buf2, m_Nsize, m_bc, NC);
    }else{
      BridgeVec::mult_wilson_tm2_chiral(v2, buf2, m_Nsize, m_bc, NC);
    }
  }
