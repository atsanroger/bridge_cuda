...
namespace{
  ...
  real_t pythag(real_t a, real_t b)  // Cf. Numerical Recipe
  {
    real_t absa, absb, v;
    absa = abs(a);
    absb = abs(b);
...
//====================================================================
template<typename FIELD, typename FOPR>
void AEigensolver_IRArnoldi<FIELD, FOPR>::get_parameters(
                                                 ...
                                                 double Vthreshold)
{
  ...
  m_Enorm_almost = 1.e-12; // this is an experimental value

  //- print input parameters
  vout.general(m_vl, "%s set_parameters:\n", class_name.c_str());
  ...
  vout.general(m_vl, "  Enorm_almost = %16.8e\n", m_Enorm_almost);
...
#pragma omp barrier
      ...
      //
      if ((Kthreshold > 0) && (Kalmost == 0)) converged = true;
      if (iter == Niter_eigen) converged = true;
...
#pragma omp barrier
    ...
    sort_eigenmodes(TDa, vk, m_TDa2, m_B, diff2_ev);

    vout.general(m_vl, "\n");
    vout.general(m_vl, "final eigenmodes:\n");
    for (int ik = 0; ik < Nk; ++ik) {
      m_fopr->mult(m_v, vk[ik]);
      axpy(m_v, -TDa[ik], vk[ik]);
      real_t vknorm = vk[ik].norm2();
      real_t diff2  = m_v.norm2() / vknorm;
      vout.general(m_vl, " %4d  (%12.8f, %12.8f)  %12.8f  %12.4e\n",
                   ik, real(TDa[ik]), imag(TDa[ik]), abs(TDa[ik]), diff2);
    }
...
//====================================================================
template<typename FIELD, typename FOPR>
void AEigensolver_IRArnoldi<FIELD, FOPR>::sort_eigenmodes(
                                        std::vector<complex_t>& TDa,
                                        std::vector<FIELD>& vk,
                                        std::vector<complex_t>& TDa2,
                                        std::vector<FIELD>& B,
                                        std::vector<real_t>& diff2)
{
  std::vector<int> reorder(m_Nk);
  ...
  for(int ik = 0; ik < m_Nk; ++ik){
    int ikp = reorder[ik];
    complex_t TD_tmp  = TDa2[ikp];
    int       ik_tmp  = ikp;
    int       ik2_tmp = ik;

    for(int ik2 = ik+1; ik2 < m_Nk; ++ik2){
      int ik2p = reorder[ik2];
      if (m_sorter->comp(TDa2[ik2p], TD_tmp) &&
                                     diff2[ik2p] < m_Enorm_eigen) {
        TD_tmp  = TDa2[ik2p];
        ik_tmp  = ik2p;
        ik2_tmp = ik2;
      }
    ...
    if(ik_tmp != reorder[ik]){
      for(int ik2 = ik2_tmp; ik2 > ik; --ik2){
        reorder[ik2] = reorder[ik2-1];
      }
      reorder[ik] = ik_tmp;
    }
  }
  ...
  int num_conv = 0;
  for(int ik = 0; ik < m_Nk; ++ik){
    if(diff2[reorder[ik]] < m_Enorm_eigen) num_conv++;
  }

  for(int ik = num_conv; ik < m_Nk; ++ik){
    int ikp = reorder[ik];
    complex_t TD_tmp  = TDa2[ikp];
    int       ik_tmp  = ikp;
    int       ik2_tmp = ik;
  ...
  if(ith == 0){
    for(int ik = 0; ik < m_Nk; ++ik){
      TDa[ik] = TDa2[reorder[ik]];
    }
  } // if(ith == 0)
#pragma omp barrier

  for(int ik = 0; ik < m_Nk; ++ik){
    copy(vk[ik], B[reorder[ik]]);
  }
...
//====================================================================
template<typename FIELD, typename FOPR>
void AEigensolver_IRArnoldi<FIELD, FOPR>::qrtrf1(
                         std::vector<complex_t>& Ht,
