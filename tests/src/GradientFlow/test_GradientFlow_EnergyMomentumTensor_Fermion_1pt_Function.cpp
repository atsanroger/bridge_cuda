/*!
        @file    test_GradientFlow_EnergyMomentumTensor_Fermion_1pt_Function.cpp
...
#include "Measurements/Fermion/fermionFlow_1pt_Function.h"
...
//  const std::string test_name = "Test_EnergyMomentumTensor_Fermion_1pt_Function";
  const std::string test_name = "GradientFlow.EnergyMomentumTensor.Fermion.1pt_Function";

  //- test-private parameters
  namespace {
    const std::string filename_input = "test_GradientFlow_EnergyMomentumTensor_Fermion_1pt_Function.yaml";
  }
  ...
  int update(int iconf)
  {
    ...
    const Parameters params_test = params_all.lookup("Test_EnergyMomentumTensor_Fermion_1pt_Function");
    ...
    const Parameters params_f_1pt_fn      = params_all.lookup("FermionFlow_1pt_Function");
    const Parameters params_source_random = params_all.lookup("SourceRandom");
    ...
    if (str_gconf_status == "Continue") {
      ...
      RandomNumberManager::restore_state(rand_readfile);
      ...
      num << "_conf" << std::setw(4) << std::setfill('0') << iconf;
      string rand_writefile_iconf = rand_writefile + num.str();
      RandomNumberManager::save_state(rand_writefile_iconf);
      vout.general(vl, " Wrote random seed to rand_writefile_iconf = %s\n", rand_writefile_iconf.c_str());
    ...
    FermionFlow_1pt_Function measurement(fprop, action_G.get(), &params_clover);
    measurement.set_parameters(params_f_1pt_fn, params_gflow, params_fflow, params_source_random);
    ...
    //measurement.measure_adjoint_flow(U);
    double result = measurement.measure_disconnected(U);

    timer.report();

    if (Communicator::nodeid() == 0) {
      RandomNumberManager::save_state(rand_writefile);
      iconf++;
      std::ostringstream num;
      num << "_conf" << std::setw(4) << std::setfill('0') << iconf;
      string rand_writefile_iconf = rand_writefile + num.str();
