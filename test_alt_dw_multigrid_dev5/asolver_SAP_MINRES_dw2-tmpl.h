/*!
      @file    asolver_SAP_MINRES_dw2-tmpl.h
...
const std::string ASolver_SAP_MINRES_dw2<AFIELD>::class_name
  = "ASolver_SAP_MINRES_dw2";

//====================================================================
template<typename AFIELD>
void ASolver_SAP_MINRES_dw2<AFIELD>::init(void)
{
...
//====================================================================
template<typename AFIELD>
void ASolver_SAP_MINRES_dw2<AFIELD>::tidyup(void)
{
  // ThreadManager::assert_single_thread(class_name);
...
//====================================================================
template<typename AFIELD>
void ASolver_SAP_MINRES_dw2<AFIELD>::set_parameters(const Parameters& params)
{
...
//====================================================================
template<typename AFIELD>
void ASolver_SAP_MINRES_dw2<AFIELD>::set_parameters(const int Niter,
...
//====================================================================
template<typename AFIELD>
void ASolver_SAP_MINRES_dw2<AFIELD>::solve(AFIELD& x, const AFIELD& b,
...
#pragma omp barrier // fine
  ...
  {
    double r2=m_r.norm2();
    vout.detailed(m_vl, "%s: |source|^2 = %23.15e\n", class_name.c_str(), r2);
    if(r2<1e-30){
      return;
    }
  }
...
#pragma omp barrier // coarse
  ...
  for (int iter = 0; iter < m_Niter; ++iter) {
    m_PV_solver->solve(m_v, m_r, nconv_PV, diff_PV, eo);
    m_fopr->mult_sap(m_p, m_v, eo);
...
#pragma omp barrier
    if(iter==0 || iter==m_Niter-1){
      double r2tmp=m_r.norm2();
      double p2tmp=m_p.norm2();
#pragma omp master
      {
        double r2 = 0.0;
        double p2 = 0.0;
        ...
        for (int i = 0; i < m_block_index->coarse_nvol(); ++i) {
          //        printf(" %d /%d    %d    %e   %e\n", i, m_block_index->coarse_nvol(), m_block_index->block_eo(i),
          //               m_r2_block[i], m_p2_block[i]);
          if (m_block_index->block_eo(i) == eo || eo < 0 ) {
            r2 += m_r2_block[i];
            p2 += m_p2_block[i];
          }
        }
        double r2sum = Communicator::reduce_sum(r2);
        double p2sum = Communicator::reduce_sum(p2);
        //        vout.general(m_vl, "hoge: eo=%d, iter=%d , sum(r2)=%e  r.norm2()=%e; sum(p2)=%e p.norm2()=%e\n", eo, iter, r2sum, r2tmp, p2sum, p2tmp);
      }// master
    } 
#pragma omp barrier
#endif // DEBUG_MINRES
  ...
  //  m_p.set(0.0);
  //  m_fopr->mult_sap(m_p, x, eo);
  //  copy(x, m_p);

#ifdef DEBUG_MINRES
  {
    m_fopr->set_mode("D");
    m_PV_solver->solve(m_v, x, nconv_PV, diff_PV, eo);
    m_fopr->mult_sap(m_r, m_v, eo);
#pragma omp master
    {
      for (int i = 0; i < m_block_index->coarse_nvol(); ++i) {
        m_alpha_block[i]=1.0;
...
#pragma omp barrier
    block_axpy_eo(m_r, &(m_alpha_block[0]), b, eo, real_t(-1.0), *m_block_index);
    block_norm2_eo(&(m_r2_block[0]), m_r, eo, *m_block_index);
    block_norm2_eo(&(m_p2_block[0]), b, eo, *m_block_index);
#pragma omp master
    {
      double r2 = 0.0;
      double b2 = 0.0;
      ...
      double r2sum = Communicator::reduce_sum(r2);
      double b2sum = Communicator::reduce_sum(b2);
      vout.general(m_vl, "minres2 is done: sum(r2)=%23.15e relative diff2=%23.15e \n",
                   r2sum, r2sum/b2sum);
...
{
  solve(x, b, Nconv, diff, -1);
}

//====================================================================
template<typename AFIELD>
double ASolver_SAP_MINRES_dw2<AFIELD>::flop_count()
{
...
//====================================================================
template<typename AFIELD>
void ASolver_SAP_MINRES_dw2<AFIELD>::calc_flop_each()
{
  int Nin  = m_fopr->field_nin();
  int Nvol = m_fopr->field_nvol();
  int Nex  = m_fopr->field_nex();
