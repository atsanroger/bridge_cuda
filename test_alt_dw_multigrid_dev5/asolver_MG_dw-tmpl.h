...
void ASolver_MG_dw<AFIELD>::init_coarse_grid()
{
  ...
  ASolver_SAP_MINRES_dw<AFIELD_f> *solver_in_sap
    = new ASolver_SAP_MINRES_dw<AFIELD_f>(afopr_fineF, m_multigrid->get_block_index());
  //  ASolver_SAP_MINRES_dw2<AFIELD_f> *solver_in_sap
  //    = new ASolver_SAP_MINRES_dw2<AFIELD_f>(afopr_fineF, m_multigrid->get_block_index());
  ...
  params_solver_in_sap.set_string("verbose_level", "General");
  solver_in_sap->set_parameters(params_solver_in_sap);
  m_asolver_in_sap.reset(solver_in_sap);

  vout.general("%s: creating the smoother: stop_cond=%e\n", class_name.c_str(), m_smoother_stop_cond);

  Smoother_t *asolver_smoother =
    new Smoother_t(m_afopr_smoother, m_multigrid->get_block_index(), m_asolver_in_sap.get(), m_asolver_PV.get());
#else
  ...
  params_smoother.set_int("maximum_number_of_iteration", m_smoother_niter);
  params_smoother.set_int("maximum_number_of_restart", 1);
  params_smoother.set_string("initial_guess_mode", "ZERO");

  ASolver_GMRES_m_Cmplx<AFIELD_f>  *asolver_gmres = dynamic_cast<ASolver_GMRES_m_Cmplx<AFIELD_f>* >(asolver_smoother);
  if(asolver_gmres != nullptr){
