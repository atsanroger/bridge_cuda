/*!
      @file    asolver_SAP_MINRES_dw-tmpl.h
...
const std::string ASolver_SAP_MINRES_dw<AFIELD>::class_name
  = "ASolver_SAP_MINRES_dw";

//====================================================================
template<typename AFIELD>
void ASolver_SAP_MINRES_dw<AFIELD>::init(void)
{
...
//====================================================================
template<typename AFIELD>
void ASolver_SAP_MINRES_dw<AFIELD>::tidyup(void)
{
  // ThreadManager::assert_single_thread(class_name);
...
//====================================================================
template<typename AFIELD>
void ASolver_SAP_MINRES_dw<AFIELD>::set_parameters(const Parameters& params)
{
...
//====================================================================
template<typename AFIELD>
void ASolver_SAP_MINRES_dw<AFIELD>::set_parameters(const int Niter,
...
//====================================================================
template<typename AFIELD>
void ASolver_SAP_MINRES_dw<AFIELD>::solve(AFIELD& x, const AFIELD& b,
...
#pragma omp barrier // fine
  ...
  m_fopr->mult_sap(m_r, b, eo);
  //copy(m_r, b);
  {
    double r2=m_r.norm2();
    vout.detailed(m_vl, "%s: |Ddag source|^2 = %23.15e\n", class_name.c_str(), r2);
    if(r2<1e-30){
      return;
    }
  }
...
#pragma omp master
    {
      ...
      vout.general(m_vl, "minres is done: sum(r2)=%23.15e relative diff2=%23.15e \n",
                   r2sum, r2sum/b2sum);
...
//====================================================================
template<typename AFIELD>
double ASolver_SAP_MINRES_dw<AFIELD>::flop_count()
{
...
//====================================================================
template<typename AFIELD>
void ASolver_SAP_MINRES_dw<AFIELD>::calc_flop_each()
{
  int Nin  = m_fopr->field_nin();
  int Nvol = m_fopr->field_nvol();
  int Nex  = m_fopr->field_nex();
