...
#include "testvector_eigen-tmpl.h"
#include "testvector_eigen_block-tmpl.h"
...
// class name
const std::string Test_Coarse::class_name = "Test_Coarse";
...
int Test_Coarse::test()
{
  ...
  AFopr_Domainwall_5din_dd<AFIELD_f> *afopr_fineF =
  ...
  // setting testvectors with blocked eigensolver (double)
  {
    vout.crucial("Setting testvectors by eigenmodes\n");

    AFopr_Domainwall_5din_dd<AFIELD_d>* afopr_d =
              new AFopr_Domainwall_5din_dd<AFIELD_d>(params_fopr_dd);
    afopr_d->set_config(U.get());
    //afopr_d->set_mode("D");
    ...
    typedef typename ComplexTraits<AFIELD_f::real_t>::complex_t
                                                      complex_f_t;

    std::vector<AFIELD_f> testvectors(num_vectors);
    int Nin  = afopr_fineF->field_nin();
    int Nvol = afopr_fineF->field_nvol();
    int Nex  = afopr_fineF->field_nex();
    for(int ik = 0; ik < num_vectors; ++ik){
      testvectors[ik].reset(Nin, Nvol, Nex);
    }

    std::vector<complex_f_t> TDa(Nblock * num_vectors);

    Parameters params_fprop = params_all.lookup("Fprop");

    string fprop_type = params_fprop.get_string("fprop_type");
    if(fprop_type != "Multigrid"){
      vout.crucial(vl, "unsupported fprop_type = %s\n",
		   fprop_type.c_str());
      exit(EXIT_FAILURE);
    }

    string testvector_type = params_fprop.get_string("testvector_type");
    if(testvector_type != "eigenvector"){
      vout.crucial(vl, "unsupported testvector_type = %s\n",
		   fprop_type.c_str());
      exit(EXIT_FAILURE);
    }
    ...
    Parameters params_eigen = params_fprop.lookup("Eigen_params");
    params_eigen.set_int_vector("fine_lattice", fine_lattice);
    params_eigen.set_int_vector("coarse_lattice", coarse_lattice);

    string fopr_mode = params_eigen.get_string("fopr_mode");
    vout.general("fopr_mode = %s\n", fopr_mode.c_str());
    afopr_fineF->set_mode(fopr_mode);
    afopr_d->set_mode(fopr_mode);

    string filename_base = params_eigen.get_string("filename_base");

    string eigensolver_type = params_eigen.get_string("eigensolver_type");

    if(eigensolver_type == "IRLanczos"){

      //set_testvectors_eigen(afopr_d, testvectors, coarse_lattice,
      //                      params_eigen, filename_base);

      read_eigenvectors(afopr_fineF, TDa, atestvec, filename_base);

      check_eigenvectors(afopr_fineF, TDa, atestvec);

      /*
      typedef typename ComplexTraits<AFIELD_d::real_t>::complex_t
                                                      complex_d_t;
      std::vector<complex_d_t> TDa_d(num_vectors);
      std::vector<AFIELD_d> atestvec_d(num_vectors);
      int Nin  = afopr_d->field_nin();
      int Nvol = afopr_d->field_nvol();
      int Nex  = afopr_d->field_nex();
      for(int ik = 0; ik < num_vectors; ++ik){
        atestvec_d[ik].reset(Nin, Nvol, Nex);
      }

      read_eigenvectors(afopr_d, TDa_d, atestvec_d, filename_base);
      check_eigenvectors(afopr_d, TDa_d, atestvec_d);
      */
    ...
    }else if(eigensolver_type == "IRArnoldi_block"){

      //set_testvectors_eigen_block(afopr_d, testvectors, coarse_lattice,
      //                            params_eigen, filename_base);

      read_eigenvectors_block(afopr_fineF, TDa, atestvec, coarse_lattice,
                              filename_base);

      check_eigenvectors_block(afopr_fineF, TDa, atestvec, coarse_lattice);
    }

    delete afopr_d;

    Communicator::sync();
  }    // setting testvectors end
