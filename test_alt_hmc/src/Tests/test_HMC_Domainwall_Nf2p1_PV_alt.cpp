...
namespace Test_HMC_Domainwall {
  const std::string test_name = "HMC.Domainwall.Nf2p1_PV";

  //- test-private parameters
  namespace {
    const std::string filename_input
                     = "test_HMC_Domainwall_Nf2p1_PV_alt.yaml";
  }
  ...
  int update_Nf2p1_PV_alt(void)
  {
    vout.crucial("test update_Nf2p1_PV_alt start.\n");
    ...
    unique_ptr<AFopr<AFIELD> > fopr_dw(
                           AFopr<AFIELD>::New("Domainwall", params_dw));

    unique_ptr<AForce_F<AFIELD> > force_fopr_dw(
                            new AForce_F_Domainwall<AFIELD>(params_dw));
    ...
    unique_ptr<Fprop_alt<AFIELD> > fprop_MD(
      new Fprop_alt_Standard_lex<AFIELD>(params_dw, params_solver_MD,
                                         dr_smear.get()));

    unique_ptr<Fprop_alt<AFIELD> > fprop_H(
      new Fprop_alt_Standard_lex<AFIELD>(params_dw, params_solver_H,
                                         dr_smear.get()));

    unique_ptr<AFopr<AFIELD> > fopr_dw_pv(
                         AFopr<AFIELD>::New("Domainwall", params_dw_pv));

    unique_ptr<AForce_F<AFIELD> > force_fopr_dw_pv(
                          new AForce_F_Domainwall<AFIELD>(params_dw_pv));
    ...
    unique_ptr<Fprop_alt<AFIELD> > fprop_H_pv(
      new Fprop_alt_Standard_lex<AFIELD>(params_dw_pv, params_solver_H,
                                         dr_smear.get()));

    unique_ptr<Action> action_F(new Action_F_alt_Ratio_lex<AFIELD>(
    ...
    unique_ptr<AForce_F<AFIELD> > force_fopr1_Nf1(
      new AForce_F_Domainwall<AFIELD>(params_fopr1_Nf1));
    ...
    unique_ptr<AForce_F<AFIELD> > force_fopr2_Nf1(
      new AForce_F_Domainwall<AFIELD>(params_fopr2_Nf1));
