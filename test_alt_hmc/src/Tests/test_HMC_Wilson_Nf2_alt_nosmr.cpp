...
namespace Test_HMC_Wilson {
  ...
  int update_Nf2_alt_nosmr(void)
  {
    ...
    // unique_ptr<Force> force_fopr_smear(new AForce_F_Smeared_alt<AFIELD>(
    //                                   force_fopr.get(), dr_smear.get()));
    ...
    unique_ptr<Action> action_F(new Action_F_alt_Standard_lex<AFIELD>(
