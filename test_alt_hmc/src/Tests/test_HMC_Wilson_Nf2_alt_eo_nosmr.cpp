/*!
        ...
        @version $LastChangedRevision: 2517 $
*/
...
namespace Test_HMC_Wilson {
  const std::string test_name = "HMC.Wilson_eo_nosmr.Nf2";

  //- test-private parameters
  namespace {
    const std::string filename_input = "test_HMC_Wilson_Nf2_alt_eo_nosmr.yaml";
  }
  ...
  int update_Nf2_alt_eo_nosmr(void)
  {
    ...
    if(str_fopr_type.substr(str_fopr_type.size()-3 ,3) != "_eo"){
      vout.crucial(vl, "fop_type = %s\n", str_fopr_type.c_str());
      str_fopr_type += "_eo";
      vout.crucial(vl, "fop_type = %s\n", str_fopr_type.c_str());
    }
    ...
    unique_ptr<AForce_F<AFIELD> > force_fopr(
	   ...
	   new AForce_F_Wilson_eo_Nf2<AFIELD>(params_fopr));
    ...
    unique_ptr<Fprop_alt<AFIELD> > fprop_MD(
      new Fprop_alt_Standard_eo<AFIELD>(params_fopr, params_solver_MD));
    //new Fprop_alt_Standard_eo<AFIELD>(params_fopr, params_solver_MD,
    //                                  dr_smear.get()));

    unique_ptr<Fprop_alt<AFIELD> > fprop_H(
      new Fprop_alt_Standard_eo<AFIELD>(params_fopr, params_solver_H));
    // new Fprop_alt_Standard_eo<AFIELD>(params_fopr, params_solver_H,
    //                                    dr_smear.get()));
