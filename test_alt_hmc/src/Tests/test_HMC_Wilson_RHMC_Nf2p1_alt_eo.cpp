...
namespace Test_HMC_Wilson {
  const std::string test_name = "HMC.Wilson.RHMC_Nf2p1_alt_eo";

  //- test-private parameters
  namespace {
    const std::string filename_input = "test_HMC_Wilson_RHMC_Nf2p1_alt_eo.yaml";
  }
  ...
  int RHMC_Nf2p1_alt_eo(void)
  {
    ...
    unique_ptr<AFopr<AFIELD> > fopr_Nf1(
           AFopr<AFIELD>::New(str_fopr_Nf1_type, params_fopr_Nf1));

    unique_ptr<AForce_F<AFIELD> > force_fopr_Nf1(
           new AForce_F_Wilson_eo_Nf2<AFIELD>(params_fopr_Nf1));
           //new AForce_F_Wilson_Nf2<AFIELD>(params_fopr_Nf1));

    //- NB. Langevin needs params_rational_H
    unique_ptr<AFopr<AFIELD> > fopr_r1(
           AFopr<AFIELD>::New("Rational", fopr_Nf1.get(), params_rational_H));

    unique_ptr<AFopr<AFIELD> > fopr_smear_Nf1_langev(
           AFopr<AFIELD>::New("Smeared_alt", fopr_r1.get(), dr_smear.get()));

    //- NB. Hamiltonian needs params_rational_MD
    unique_ptr<AFopr<AFIELD> > fopr_r2(
           AFopr<AFIELD>::New("Rational", fopr_Nf1.get(), params_rational_MD));

    unique_ptr<AFopr<AFIELD> > fopr_smear_Nf1_H(
           AFopr<AFIELD>::New("Smeared_alt", fopr_r2.get(), dr_smear.get()));

    unique_ptr<AForce_F<AFIELD> > force_fopr_Nf1_MD(
           new AForce_F_Rational<AFIELD>(fopr_Nf1.get(), force_fopr_Nf1.get(),
                                         params_rational_MD));

    unique_ptr<AForce_F<AFIELD> > force_fopr_smear_Nf1_MD(
           new AForce_F_Smeared_alt<AFIELD>(force_fopr_Nf1_MD.get(),
                                            dr_smear.get()));
    unique_ptr<Action> action_F_Nf1(
           new Action_F_alt_Rational<AFIELD>(fopr_smear_Nf1_langev.get(),
                                             fopr_smear_Nf1_H.get(),
                                             force_fopr_smear_Nf1_MD.get()));
