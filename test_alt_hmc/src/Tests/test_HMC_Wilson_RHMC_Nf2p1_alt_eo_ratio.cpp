/*!
        @file    test_HMC_Wilson_RHMC_Nf2p1_alt.cpp
...
#include "lib_alt/Action/Fermion/action_F_alt_Rational.h"
...
namespace Test_HMC_Wilson {
  const std::string test_name = "HMC.Wilson.RHMC_Nf2p1_alt_eo_ratio";

  //- test-private parameters
  namespace {
    const std::string filename_input = "test_HMC_Wilson_RHMC_Nf2p1_alt_eo_ratio.yaml";
  }
  ...
  int RHMC_Nf2p1_alt_eo_ratio(void)
  {
    ...
    const Parameters params_fopr1_Nf1           = params_all.lookup("Fopr1_Nf1");
    //const Parameters params_proj               = params_all.lookup("Projection");
    const Parameters params_fopr2_Nf1           = params_all.lookup("Fopr2_Nf1");
    //const Parameters params_proj               = params_all.lookup("Projection");
    ...
    const Parameters params_rational1_MD        = params_all.lookup("Fopr_Rational1_MD");
    const Parameters params_rational1_H         = params_all.lookup("Fopr_Rational1_H");
    const Parameters params_rational2_MD        = params_all.lookup("Fopr_Rational2_MD");
    const Parameters params_rational2_H         = params_all.lookup("Fopr_Rational2_H");
    ...
    //-- N_f=2 part
    unique_ptr<AFopr<AFIELD> > fopr_Nf2(AFopr<AFIELD>::New(
                                     str_fopr_Nf2_type, params_fopr_Nf2));

    unique_ptr<AForce_F<AFIELD> > force_fopr_Nf2(
           new AForce_F_Wilson_Nf2<AFIELD>(params_fopr_Nf2));

    unique_ptr<AFopr<AFIELD> > fopr_smear_Nf2(
           AFopr<AFIELD>::New("Smeared_alt", fopr_Nf2.get(), dr_smear.get()));

    unique_ptr<AForce_F<AFIELD> > force_fopr_smear_Nf2(
           new AForce_F_Smeared_alt<AFIELD>(force_fopr_Nf2.get(),
                                            dr_smear.get()));

    unique_ptr<Fprop_alt<AFIELD> > fprop_Nf2_MD(
           new Fprop_alt_Standard_lex<AFIELD>(params_fopr_Nf2,
                                              params_solver_Nf2_MD,
                                              dr_smear.get()));

    unique_ptr<Fprop_alt<AFIELD> > fprop_Nf2_H(
           new Fprop_alt_Standard_lex<AFIELD>(params_fopr_Nf2,
                                              params_solver_Nf2_H,
                                              dr_smear.get()));
    ...
    unique_ptr<AFopr<AFIELD> > fopr_Nf2_prec(
           AFopr<AFIELD>::New(str_fopr_Nf2_prec_type, params_fopr_Nf2_prec));

    unique_ptr<AForce_F<AFIELD> > force_fopr_Nf2_prec(
           new AForce_F_Wilson_Nf2<AFIELD>(params_fopr_Nf2_prec));

    unique_ptr<AFopr<AFIELD> > fopr_smear_Nf2_prec(
           AFopr<AFIELD>::New("Smeared_alt",
                              fopr_Nf2_prec.get(), dr_smear.get()));

    unique_ptr<AForce_F<AFIELD> > force_fopr_smear_Nf2_prec(
           new AForce_F_Smeared_alt<AFIELD>(
                              force_fopr_Nf2_prec.get(), dr_smear.get()));

    unique_ptr<Fprop_alt<AFIELD> > fprop_Nf2_MD_prec(
           new Fprop_alt_Standard_lex<AFIELD>(params_fopr_Nf2_prec,
                                              params_solver_Nf2_MD_prec,
                                              dr_smear.get()));

    unique_ptr<Fprop_alt<AFIELD> > fprop_Nf2_H_prec(
           new Fprop_alt_Standard_lex<AFIELD>(params_fopr_Nf2_prec,
                                              params_solver_Nf2_H_prec,
                                              dr_smear.get()));

    unique_ptr<Action> action_F_Nf2_prec(
           new Action_F_alt_Standard_lex<AFIELD>(
                                           fopr_smear_Nf2_prec.get(),
                                           force_fopr_smear_Nf2_prec.get(),
                                           fprop_Nf2_MD_prec.get(),
                                           fprop_Nf2_H_prec.get()));

    unique_ptr<Action> action_F_Nf2_d(
        new Action_F_alt_Ratio_lex<AFIELD>(fopr_smear_Nf2_prec.get(),
                                           force_fopr_smear_Nf2_prec.get(),
                                           fopr_smear_Nf2.get(),
                                           force_fopr_smear_Nf2.get(),
                                           fprop_Nf2_H_prec.get(),
                                           fprop_Nf2_MD.get(),
                                           fprop_Nf2_H.get()));

    //-- N_f=1 part (ratio)
    string fopr1_Nf1_type = params_fopr1_Nf1.get_string("fermion_type");
    unique_ptr<AFopr<AFIELD> > fopr1_Nf1(
           AFopr<AFIELD>::New(fopr1_Nf1_type, params_fopr1_Nf1));

    unique_ptr<AForce_F<AFIELD> > force_fopr1_Nf1(
           new AForce_F_Wilson_eo_Nf2<AFIELD>(params_fopr1_Nf1));
           //new AForce_F_Wilson_Nf2<AFIELD>(params_fopr_Nf1));

    unique_ptr<AFopr<AFIELD> > fopr1_r1(
           AFopr<AFIELD>::New("Rational", fopr1_Nf1.get(), params_rational1_H));

    unique_ptr<AFopr<AFIELD> > fopr1_smear_Nf1_langev(
           AFopr<AFIELD>::New("Smeared_alt", fopr1_r1.get(), dr_smear.get()));

    unique_ptr<AFopr<AFIELD> > fopr1_r2(
           AFopr<AFIELD>::New("Rational", fopr1_Nf1.get(), params_rational1_MD));

    unique_ptr<AFopr<AFIELD> > fopr1_smear_Nf1_H(
           AFopr<AFIELD>::New("Smeared_alt", fopr1_r2.get(), dr_smear.get()));

    unique_ptr<AForce_F<AFIELD> > force_fopr1_Nf1_MD(
      new AForce_F_Rational<AFIELD>(fopr1_Nf1.get(), force_fopr1_Nf1.get(),
                                         params_rational1_MD));

    unique_ptr<AForce_F<AFIELD> > force_fopr1_smear_Nf1_MD(
      new AForce_F_Smeared_alt<AFIELD>(force_fopr1_Nf1_MD.get(),
                                            dr_smear.get()));


    string fopr2_Nf1_type = params_fopr2_Nf1.get_string("fermion_type");
    unique_ptr<AFopr<AFIELD> > fopr2_Nf1(
           AFopr<AFIELD>::New(fopr2_Nf1_type, params_fopr2_Nf1));

    unique_ptr<AForce_F<AFIELD> > force_fopr2_Nf1(
           new AForce_F_Wilson_eo_Nf2<AFIELD>(params_fopr2_Nf1));
           //new AForce_F_Wilson_Nf2<AFIELD>(params_fopr_Nf1));

    //- NB. Langevin needs params_rational_H
    unique_ptr<AFopr<AFIELD> > fopr2_r1(
           AFopr<AFIELD>::New("Rational", fopr2_Nf1.get(), params_rational2_H));

    unique_ptr<AFopr<AFIELD> > fopr2_smear_Nf1_langev(
           AFopr<AFIELD>::New("Smeared_alt", fopr2_r1.get(), dr_smear.get()));

    //- NB. Hamiltonian needs params_rational_MD
    unique_ptr<AFopr<AFIELD> > fopr2_r2(
           AFopr<AFIELD>::New("Rational", fopr2_Nf1.get(), params_rational2_MD));

    unique_ptr<AFopr<AFIELD> > fopr2_smear_Nf1_H(
           AFopr<AFIELD>::New("Smeared_alt", fopr2_r2.get(), dr_smear.get()));

    unique_ptr<AForce_F<AFIELD> > force_fopr2_Nf1_MD(
           new AForce_F_Rational<AFIELD>(fopr2_Nf1.get(), force_fopr2_Nf1.get(),
                                         params_rational2_MD));

    unique_ptr<AForce_F<AFIELD> > force_fopr2_smear_Nf1_MD(
           new AForce_F_Smeared_alt<AFIELD>(force_fopr2_Nf1_MD.get(),
                                            dr_smear.get()));

    unique_ptr<Action> action_F_Nf1(
           new Action_F_alt_Rational_Ratio<AFIELD>(
                                          fopr1_smear_Nf1_langev.get(),
                                          fopr1_smear_Nf1_H.get(),
                                          force_fopr1_smear_Nf1_MD.get(),
                                          fopr2_smear_Nf1_langev.get(),
                                          fopr2_smear_Nf1_H.get(),
                                          force_fopr2_smear_Nf1_MD.get()));
    ...
    for (int traj = 0; traj < Ntraj; ++traj) {
      ...
      if ((i_conf + traj + 1) % i_save_conf == 0) {
        std::string filename =
          FileUtils::generate_filename("%s-%06d", writefile.c_str(),
                                       (i_conf + traj + 1));
