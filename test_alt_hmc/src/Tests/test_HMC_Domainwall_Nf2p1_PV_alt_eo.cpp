...
namespace Test_HMC_Domainwall {
  const std::string test_name = "HMC.Domainwall_eo.Nf2p1_PV";

  //- test-private parameters
  namespace {
    const std::string filename_input
                     = "test_HMC_Domainwall_Nf2p1_PV_alt_eo.yaml";
  }
  ...
  int update_Nf2p1_PV_alt_eo(void)
  {
    vout.crucial("test update_Nf2_PV_alt_eo start.\n");
    ...
    const Parameters params_pv      = params_all.lookup("Fopr_Domainwall_PauliVillars");
    ...
    unique_ptr<AForce_F<AFIELD> > forceF_dw(
                          new AForce_F_Domainwall_eo<AFIELD>(params_dw));

    unique_ptr<AFopr<AFIELD> >  fopr_dw_smr(
       AFopr<AFIELD>::New("Smeared_alt", fopr_dw.get(), dr_smear.get()));

    unique_ptr<AForce_F<AFIELD>> forceF_dw_smr(new
       AForce_F_Smeared_alt<AFIELD>(forceF_dw.get(), dr_smear.get()));

    unique_ptr<Fprop_alt<AFIELD> > fprop_dw_MD(
      new Fprop_alt_Standard_eo<AFIELD>(params_dw, params_solver_MD,
                                        dr_smear.get()));

    unique_ptr<Fprop_alt<AFIELD> > fprop_dw_H(
      new Fprop_alt_Standard_eo<AFIELD>(params_dw, params_solver_H,
                                        dr_smear.get()));

    //- Pauli-Villars part
    unique_ptr<AFopr<AFIELD> > fopr_pv(
                         AFopr<AFIELD>::New("Domainwall_eo", params_pv));

    unique_ptr<AForce_F<AFIELD> > forceF_pv(
                        new AForce_F_Domainwall_eo<AFIELD>(params_pv));

    unique_ptr<AFopr<AFIELD> >  fopr_pv_smr(
       AFopr<AFIELD>::New("Smeared_alt", fopr_pv.get(), dr_smear.get()));

    unique_ptr<AForce_F<AFIELD> > forceF_pv_smr(new
       AForce_F_Smeared_alt<AFIELD>(forceF_pv.get(), dr_smear.get()));

    unique_ptr<Fprop_alt<AFIELD> > fprop_pv_H(
      new Fprop_alt_Standard_eo<AFIELD>(params_pv, params_solver_H,
                                        dr_smear.get()));

    //- Domain-sall/Pauli-Villars action
    unique_ptr<Action> action_F(new Action_F_alt_Ratio_eo<AFIELD>(
                             fopr_pv_smr.get(), forceF_pv_smr.get(),
                             fopr_dw_smr.get(), forceF_dw_smr.get(),
                             fprop_pv_H.get(),
                             fprop_dw_MD.get(), fprop_dw_H.get()));
    ...
    unique_ptr<AForce_F<AFIELD> > force_fopr1_Nf1(
      new AForce_F_Domainwall_eo<AFIELD>(params_fopr1_Nf1));
    ...
    unique_ptr<AForce_F<AFIELD> > force_fopr2_Nf1(
      new AForce_F_Domainwall_eo<AFIELD>(params_fopr2_Nf1));
